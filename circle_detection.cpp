...
void drawCircles(cv::Mat &input, const std::vector<cv::Vec3f> &circles, cv::Scalar color) {
   for (int i = 0; i < circles.size(); i++) //Drawing the circles
    {
        ...
        int radius = c[2];
        circle(input, center, radius, color, 3, cv::LINE_AA);

        std::cout << "Drew Point at " << center << " with r=" << radius << "    Lenght:" << circles.size() <<std::endl;
    }
}

std::vector<cv::Vec3f> findCircles(cv::Mat &input, cv::Mat &inputDrawOn, const std::pair<int, int> &detectionSizes, cv::Scalar color) {
    ...
    drawCircles(inputDrawOn, circles, color);
